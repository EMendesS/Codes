...
void PeripheralInit_GPIO() {
    ...
    GPIO_InitDef.GPIO_Pin = GPIO_Pin_13 | GPIO_Pin_15;         // GPIO pin 13
...
void PeripheralInit_USART3(){
    ...
    // initialize USART3 TX alternate function push-pull (50 MHz)
    GPIO_InitDef.GPIO_Pin = GPIO_Pin_10;
    GPIO_InitDef.GPIO_Mode = GPIO_Mode_AF_PP;
    GPIO_InitDef.GPIO_Speed = GPIO_Speed_50MHz;
    GPIO_Init(GPIOB, &GPIO_InitDef);

    // initialize USART3 RX input floating (50 MHz)
...
// Transmit a character over USART3 B10/TX
...
// Receive a character over USART3 B11/RX
int get_char_USART3(){
...
// Transmit a string over USART3 B10/TX
...
//    volatile uint32_t count = 0;
    ...
    PeripheralInit_USART3(); // initialize USART3 at B10/TX, B11/RX
    ...
    // announce start of code over USART3
    serial_print_USART3("Quick example of SPI communication with the L3GD20H gyro.\n");
    ...
    while(1){
        ...
        numRead = transfer_8b_SPI2_Master(0xB800); // value WHO_AM_I
        ...
        if(numRead == VAL_WHO_AM_I) {
            serial_print_USART3("Master Received Value....");
            itoa(numRead, palavra, 10);
            serial_print_USART3(palavra);
            serial_print_USART3("\n");
        } else {
            serial_print_USART3("Master Received Wrong....");
            itoa(numRead, palavra, 10);
            serial_print_USART3(palavra);
            serial_print_USART3("\n");
        }
